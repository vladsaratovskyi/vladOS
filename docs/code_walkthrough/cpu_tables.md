...
- `x86_64::instructions::interrupts` for updating `rsp0` with local
  interrupts disabled
...
- The IST pointer must point to the top of the stack because x86_64 stacks grow
  downward.
- `TSS.rsp0` must name the selected task's kernel-stack top before the CPU can
  safely enter the kernel from CPL3.
...
| `use x86_64::instructions::interrupts as cpu_interrupts;` | Lets `set_kernel_stack` update `TSS.rsp0` while local interrupts are disabled. |
...
| `use x86_64::{PrivilegeLevel, VirtAddr};` | Imports privilege levels for placeholder/user selectors and virtual-address helpers for stack addresses. |
...
| `user_code_selector: SegmentSelector,` | Selector for the ring-3 code descriptor used in user `iretq` frames. |
| `user_data_selector: SegmentSelector,` | Selector for the ring-3 data/stack descriptor used as user `SS`. |
...
| `init_gdt();` | Adds kernel, user, and TSS descriptors to the GDT. |
...
| `let user_data_selector = gdt.add_entry(Descriptor::user_data_segment());` | Adds a DPL 3 data descriptor for user stacks. |
| `let user_code_selector = gdt.add_entry(Descriptor::user_code_segment());` | Adds a DPL 3 long-mode code descriptor for user instruction pointers. |
...
| `kernel_code_selector()` and `kernel_data_selector()` | Expose the loaded ring-0 selectors for code that builds or inspects kernel trap frames. |
| `user_code_selector()` and `user_data_selector()` | Return the user selectors with RPL 3 set, which is required for an `iretq` transition to CPL3. |
| `set_kernel_stack(stack_top)` | Stores the selected task's kernel-stack top in `TSS.privilege_stack_table[0]`, the `rsp0` slot used on CPL3 to CPL0 interrupts. |
