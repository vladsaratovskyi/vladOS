...
| `name = "blog_os"` | The crate is named `blog_os`; integration tests import it as `blog_os`. |
...
| `target = "x86_64-blog_os.json"` | Uses the custom target specification instead of the host Windows target. |
| `[unstable]` | Starts nightly-only Cargo settings. |
| `build-std = ["core", "compiler_builtins"]` | Rebuilds `core` and compiler builtins for the custom target. |
