...
- User code/data/stacks are mapped with `USER_ACCESSIBLE` in per-process address
  spaces.
...
- ELF-backed user tasks use the same per-process address-space isolation as the
...
This test proves the first process layer: user processes have PIDs and
parent/child metadata, child exits become zombies, parents can wait with
`waitpid`, and user faults become waitable abnormal process exits.

### Invariants

- The scheduler still runs tasks; processes own address spaces and lifecycle
  metadata.
- A child zombie must stay in the process table until the parent reaps it.
- Blocking `waitpid` must block the parent task without busy-looping.
- Bad wait-status pointers must return `-EFAULT` without reaping the child.
- Faulting children must not kill the parent or sibling processes.

### Line-By-Line

| Code | Explanation |
| --- | --- |
| `PARENT_SUITE_ELF` | User parent program that calls `getpid` and several `waitpid` forms. |
| `spawn_user_elf_process(...)` | Creates a root user process and returns both `ProcessId` and `TaskId`. |
| `spawn_child_user_elf_process(parent.pid, ...)` | Creates children linked to the parent process. |
| `scheduler::process_parent(...)` | Verifies kernel-side parent metadata before any process runs. |
| `write_pid_table(...)` | Copies child PIDs into the parent's writable user data page before scheduling. |
| first `scheduler::yield_now()` | Lets the parent yield once and lets immediate/faulting children become zombies. |
| `process_state(immediate.pid) == Zombie(Exited(7))` | Proves a child remains in the process table before reap. |
| second `scheduler::yield_now()` | Lets the parent enter blocking wait while the delayed child is still running. |
| bounded yield loop | Gives the delayed child enough deterministic turns to exit and wake the blocked parent. |
